...
abstract contract ERC404 is IERC404 {
  ...
  // @todo
  /// @dev index of owned by value
    mapping(uint256 => uint256) internal _ownedByValueIndex;

// @todo add the _valueOfId variable  
...
// @todo add a seperate cue for each value and give this function an tokenValue argument
    ...
    for (uint256 i = start_; i < start_ + count_; ) {
      tokensInQueue[i - start_] = _storedERC721Ids._valueOfId.at(i);
...
///@todo add token value variable here for the ERC20 transfer accuracy
    // Transfer 1 * units ERC-20 and 1 ERC-721 token.
    // ERC-721 transfer exemptions handled above. Can't make it to this point if either is transfer exempt.
    uint256 tokensToTransfer = _tokenValue[id_];
    _transferERC20(from_, to_, tokensToTransfer);
    _transferERC721(from_, to_, id_);
  }
...
//@todo switch this out to include owned by type
      uint256 updatedId = _owned[from_][_owned[from_].length - 1];
      if (updatedId != id_) {
//@todo add ownedbyvalue index
        uint256 updatedIndex = _getOwnedByValueIndex(id_);
        // update _owned for sender
//@todo update new mapping including token value        
        _owned[from_][updatedIndex] = updatedId;
        // update index for the moved id
//@todo change owned index to be seperate for each cue        
        _setOwnedIndex(updatedId, updatedIndex);
      }
//@togo add tokenValue ehere
      // pop
      _owned[from_].pop();
    }
    ...
    if (to_ != address(0)) {
      ...
      _setOwnerOf(id_, to_);
//@todo add token value here  
      // Push token onto the new owner's stack.
      _owned[to_].push(id_);
      // Update index for new owner's stack.
//@todo add the add the value to the index      
...
//@todo switch this out to include owned by type
      ...
      uint256[] storage senderIdsOfValue = _ownedByValue[from][valueOfId];  
...
// @todo add loop to mint logic
      uint256 tokensToRetrieveOrMint = (balanceOf[to_] / units) -
        (erc20BalanceOfReceiverBefore / units);
 
// @todo do it for each token value
      ...
      // Only cares about whole number increments.
// @todo this here is like loop to burn - do it for each value      
      uint256 tokensToWithdrawAndStore = (erc20BalanceOfSenderBefore / units) -
        (balanceOf[from_] / units);
      for (uint256 i = 0; i < tokensToWithdrawAndStore; ) {
// @todo add value logic to withdraw adn store
...
// @todo use loop to calculate quantity of each value to go      
      uint256 nftsToTransfer = value_ / units;
      for (uint256 i = 0; i < nftsToTransfer; ) {
        // Pop from sender's ERC-721 stack and transfer them (LIFO)
// @todo pop each value from the proper stack        
      ...
      if (
//@todo revisit this when you understand better    
...
// @todo add token value to this function as an argument I think - have to be stored by value
    uint256 id;
  ...
  // Does not handle ERC-721 exemptions.
///@todo add token value to withdraw and store  
  function _withdrawAndStoreERC721(address from_) internal virtual {
    if (from_ == address(0)) {
      revert InvalidSender();
    }

//@todo make sure it's getting it from the right list
...
//@todo store it in the right list
...
// @todo have to loop through the calculation logic by value to retrieve or mint      
