...
*/
    ...
    function getOwnedTokensByValueIndex(
        ...
        require(tokenValue_ < NUM_TOKEN_VALUES, "Invalid category index");

        uint256 categoryValue = tokenValues[tokenValue_]; // Directly use the category value from the array

        uint256 count = 0;
        uint256[] memory ownedTokens = _owned[tokenHolder_];
        // First, count the tokens of the specified category to allocate memory efficiently
        for (uint256 i = 0; i < ownedTokens.length; i++) {
            if (getTokenValueFromId(ownedTokens[i]) == categoryValue) {
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < ownedTokens.length; i++) {
            ...
            if (getTokenValueFromId(tokenId) == categoryValue) {
    ...
    function getOwnedTokensSortedByValue(address tokenHolder_) 
        public 
        view 
        returns (
...
/*
    function _batchTransferERC721(
        ...
        for (uint256 i = 0; i < NUM_TOKEN_VALUES && unitsToTransfer > 0; ) {
            ...
            // get sender's balance of selected token value
            ...
            if (balance >= tokens != 0) {uint256 remainingTokens = tokens;
                } else if (balance != 0 ) {
                    uint256 remainingTokens = balance;
                  }

                for (uint256 j = 0; j < remainingTokens && unitsToTransfer >= tokenValue_;) {
                    
                    _transferERC721(from_, tokenValue_);
                    unitsToTransfer -= tokenValue_;
                    unchecked { ++j; }
                }

                // Check if change will be needed to cover remaining tokens
                if (nativeUnitsOwnedBySender_ / amountInUnits_ - balance < nftsToWithdrawAndStore 
                    && unitsToTransfer > 0) {            
                        // Make sure owner has enough tokens
                    uint256 balanceOfValue_ = getBalanceOfTokenValue(from_, tokenValue_);
                    require(balanceOfValue_ > 0, "Not enough tokens.");                
                    ...
                    uint256 changeOwedToSender = tokenValue_ > unitsToTransfer ? tokenValue_ - unitsToTransfer : 0;
                    // @todo migrate the get change function over
...
*/
    ...
    function _batchTransferOrWithdrawAndStoreERC721(
        ...
        for (uint256 i = 0; i < NUM_TOKEN_VALUES && unitsToTransfer > 0;) {
            ...
            // If balance is zero, then skip this iteration
            if (balance == 0) {
                continue;
            }
            ...
            if (tokenValue_ >= unitsToTransfer 
                && (senderNativeBalanceBefore - (balance * tokenValue_) < unitsToTransfer)) {
...
/*
    ...
    function _withdrawAndStoreBatch(
        address from_, 
        uint256 amountInUnits_, 
        ) internal {

        uint256 remainingUnitsToWithdraw = amountInUnits_;

        // Use greedy algo logic to withdraw token values from greatest > to least
        for (uint256 i = 0; i < NUM_TOKEN_VALUES && remainingUnitsToWithdraw > 0;) {
            uint256 tokenValue_ = tokenValues[i];
            
            // Get senders balance of selected token value
            uint256 balance = getBalanceOfTokenValue(from_, tokenValue_);
            
            // Determine how many ERC721 of this value to withdraw
            uint256 tokens = remainingUnitsToWithdraw / tokenValue_;

            
            uint256 remainingTokens;
            // If balance of tokenValue is less than tokens to withdraw, 
            // withdraw balance, then move onto next token value      
            if (balance >= tokens != 0) {
                ...
                // Withdraw and store remainingTokens of this value
                for (uint256 j = 0; j < remainingTokens && remainingUnitsToWithdraw >= tokenValue_; ) {
                    
                    _withdrawAndStoreERC721(from_, tokenValue_);
                    
                    // update remaining units to transfer
                    remainingUnitsToWithdraw -= tokenValue_;
                    
                    // update remaining tokens of this value to transfer
                    remainingTokens -= 1;
                    unchecked { ++j; }
                }
                ...
                if (nativeUnitsOwnedBySender_ / amountInUnits_ - (tokenValue_ * ids.length) < remainingUnitsToWithdraw 
                    && remainingUnitsToWithdraw > 0) {            
                        // Make sure owner has enough tokens
                    uint256 balanceOfValue_ = getBalanceOfTokenValue(from_, tokenValue_);
                    require(balanceOfValue_ > 0, "Not enough tokens.");                
                        // Withdraw ERC721 to cover the last part of change
                    ...
                    // Calculate change owed to sender by subtracting remaining from value of token spent
                    uint256 changeOwedToSender = tokenValue_ > nftsToWithdrawAndStore ? tokenValue_ - nftsToWithdrawAndStore : 0;
...
*/    
    ...
    function _withdrawAndStoreERC721(address from_, uint256 tokenValue_) internal override {
        ...
        uint256[] memory ids = getOwnedTokensByValueIndex(from_, tokenValue_);
    ...
    function _calculateTokensFromBatch(
        ...
        uint256 remainingUnits = unitValue_;
...
/*
    ...
    function _retrieveOrMintBatch(
        ...
        uint256[] memory _tokensToRetrieveOrMintForSender = calculateTokensFromBatch(amount_);

        // Loop through each token value & check change owed in loop condition
        for (uint256 i = 0; i < NUM_TOKEN_VALUES && amount_ > 0; ) {
            uint256 _quantity = _tokensToRetrieveOrMintForSender[i];
            uint256 tokenValue_ = tokenValues[i]; // Directly use the value from the array

            // If _quantity is zero, no need to call _retrieveOrMintERC721
            if (_quantity > 0) {
                // Loop '_quantity' times for this token value
                for (uint256 j = 0; j < _quantity && amount_ >= tokenValue_; ) {
                    // Call _retrieveOrMintERC721 for each quantity
                    _retrieveOrMintERC721(to_, tokenValue_);
                    // Update change owed
                    amount_ -= tokenValue_;
                    unchecked { ++j; }
                    }
