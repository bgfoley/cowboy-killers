...
*/
    ...
    function _transferERC20WithERC721(
        ...
        uint256[] memory tokensToWithdraw = _calculateFromTokensOwned(from_, nftsToTransfer);

        for (uint256 i = 0; i < NUM_TOKEN_VALUES; i++) {
            uint256 tokensToSendForValue = tokensToWithdraw[i];

            unchecked {
                for (uint256 j = 0; j < tokensToSendForValue; j++) {
