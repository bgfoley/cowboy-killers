...
import { Cowboy } from "../typechain-types";
  ...
  describe("Cowboy Contract", function () {
   ...
   async function deployCowboyWithERC20sAndERC721sMinted() {
  ...
  async function deployCowboyForHavingAlreadyGrantedApprovalForAllTests() {
    const f = await loadFixture(deployCowboyWithERC20sAndERC721sMinted)
  ...
  async function deployCowboyWithTokensInSecondSigner() {
    const f = await loadFixture(deployCowboy)
  ...
  async function deployCowboyWithSomeTokensTransferredToRandomAddress() {
    const f = await loadFixture(deployCowboy)
  ...
  function containsERC721ApprovalEvent(
    ...
    describe("#constructor", function () {
      it("Initializes contract and sets owner", async function () {
        const f = await loadFixture(deployCowboy)
    ...
    describe("#erc20TotalSupply", function () {
      it("Returns the correct total supply", async function () {
        const f = await loadFixture(
          deployCowboyExampleWithSomeTokensTransferredToRandomAddressdeployCowboyWithTokensInSecondSigner,
        )
    ...
    describe("ERC20 token transfer logic for triggering ERC721 transfers", function () {
      context(
        ...
        async function () {
          it("Handles the case of the receiver gaining a whole new token", async function () {
            const f = await loadFixture(
              deployCowboyWithTokensInSecondSigner,
            )
      ...
      context("Moving one or more full tokens", async function () {
        it("Transfers whole tokens without fractional impact correctly", async function () {
          const f = await loadFixture(deployCowboyWithTokensInSecondSigner)
      ...
      context("Recipient is a contract", function () {
        context("Recipient is a valid ERC721Receiver", function () {
          it("Successfully transfers a valid ERC-721", async function () {
            const f = await loadFixture(
              deployCowboyWithERC20sAndERC721sMinted,
            )
        ...
        context(
          ...
          function () {
            it("Allows an approved operator to grant specific approval for any ERC-721 token owned by the grantor", async function () {
              const f = await loadFixture(
                deployCowboyForHavingAlreadyGrantedApprovalForAllTests,
              )
