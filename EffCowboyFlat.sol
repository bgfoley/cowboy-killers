...
contract ERC404TVExt is Ownable, ERC404 {
    ...
    uint256 private constant PREFIX_LOOSIES = (1 << 255) | (0 << 253);
    uint256 private constant PREFIX_PACKS = (1 << 255) | (1 << 253);
    uint256 private constant PREFIX_CARTONS = (1 << 255) | (2 << 253);
    uint256 private constant PREFIX_MARLBORO_MEN = (1 << 255) | (3 << 253);
...
*/
    ...
    function getTokenValueFromId(uint256 tokenId) public view returns (uint256) {
    ...
    function getOwnedTokensSortedByValue(address _tokenHolder) 
        ...
        returns (
            ...
            uint256[4] memory nominalBalances
        ) 
    {
        uint256[] storage ownedTokens = _owned[_tokenHolder];
        uint256 loosisCount;
        uint256 packsCount;
        uint256 cartonsCount;
        uint256 marlboroMensCount;
        ...
        for (uint256 i = 0; i < ownedTokens.length; i++) {
            ...
            else if (value == MARLBORO_MEN) marlboroMensCount++;
        }

        // Allocate memory for arrays
        loosis = new uint256[](loosisCount);
        packs = new uint256[](packsCount);
        cartons = new uint256[](cartonsCount);
        marlboroMens = new uint256[](marlboroMensCount);

        // Reset counters for use as indices
        loosisCount = packsCount = cartonsCount = marlboroMensCount = 0;

        // Now, sort tokens into their respective arrays
        for (uint256 i = 0; i < ownedTokens.length; i++) {
            uint256 tokenId = ownedTokens[i];
            uint256 value = getTokenValueFromId(tokenId);

            if (value == LOOSIES) loosis[loosisCount++] = tokenId;
            else if (value == PACKS) packs[packsCount++] = tokenId;
            else if (value == CARTONS) cartons[cartonsCount++] = tokenId;
            else if (value == MARLBORO_MEN) marlboroMens[marlboroMensCount++] = tokenId;
        }

        // Set the counts in the nominalBalances array
        nominalBalances = [loosisCount, packsCount, cartonsCount, marlboroMensCount];
    }
    ...
    function _transferERC20WithERC721(
        ...
        if (isFromERC721TransferExempt && isToERC721TransferExempt) {
            ...
            uint256 tokensToRetrieveOrMint = (erc20BalanceOf(to_) / units) -
                (erc20BalanceOfReceiverBefore / units);
            // retrieveOrMintERC721 handles batches
            retrieveOrMintERC721(to_, tokensToRetrieveOrMint);
        } else if (isToERC721TransferExempt) {
            uint256 tokensToWithdrawAndStore = (erc20BalanceOfSenderBefore / units) -
                (erc20BalanceOf(from_) / units);
            // Assuming withdrawAndStoreERC721 is designed to handle multiple tokens if needed
            withdrawAndStoreERC721(from_, tokensToWithdrawAndStore);
        } else {
            uint256 nftsToTransfer = value_ / units;
            // Assuming withdrawAndStoreERC721 is designed to handle one token at a time
        
            withdrawAndStoreERC721(from_, nftsToTransfer); // This seems to be missing an appropriate loop or logic
            ...
            if ((erc20BalanceOfSenderBefore / units) - (erc20BalanceOf(from_) / units) > nftsToTransfer) {
                withdrawAndStoreERC721(from_, extraLoosie); // This might be a misuse; you likely intend a different logic here
            }
            ...
            if ((erc20BalanceOf(to_) / units) - (erc20BalanceOfReceiverBefore / units) > nftsToTransfer) {
                retrieveOrMintERC721(to_, extraLoosie); // Misuse of extraLoosie; likely need a different parameter or logic
            }
        }
        return true;
    }
    ...
    function retrieveOrMintERC721(address to_, uint256 amount_) internal {
        ...
        uint256[] memory tokens = _calculateTokens(amount_); 
        ...
        for (uint256 i = 0; i < NUM_TOKEN_VALUES; i++) {
            ...
            // Use unchecked for the inner loop where overflow checks are unnecessary
            unchecked {
                for (uint256 j = 0; j < tokensOfValue; j++) {
                    ...
                    if (!idsOfValue.empty()) {
                        // Preferentially use tokens from the contract's reserve if available
                        ...
                        id = _createTokenId(tokenValue_, (_mintedTokens + 1));
                    }
    ...
    function withdrawAndStoreERC721(address from_, uint256 amount_) internal {
        if (from_ == address(0)) {
            revert InvalidSender();
        }
        ...
        uint256[] memory tokensToWithdraw = calculateFromTokensOwned(from_, amount_);
    ...
    function calculateFromTokensOwned(
      ...
      //  getOwnedTokensSortedByValue(owner_);
        (, , , , uint256[4] memory nominalBalances) = getOwnedTokensSortedByValue(owner_);
    ...
    function _reinstateERC721Balance(address target_) internal override {
        uint256 _targetBalance = balanceOf[target_];
                    retrieveOrMintERC721(target_, _targetBalance);
    }
    ...
    /// @param target_ address to reinstate balances
    function clearERC721Balance(address target_) internal {
        uint256 erc721Balance = erc721BalanceOf(target_);
        withdrawAndStoreERC721(target_, erc721Balance);
    }
...
/*
        // Mint function
    function mintButt(address to, uint256 value) internal returns (uint256) {
        butts++;
        uint256 newItemId = _tokenIds;
        _mint(to, newItemId);
        tokenValue[newItemId] = value; // Assign value to the new token
        return newItemId;
    }

    // Burn function and replace with BUTT token
    function burnAndReplaceWithButt(uint256 tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Caller is not owner nor approved");

        // Determine the value of the token to be burned
        uint256 value = tokenValue[tokenId];

        // Burn the original token
        _burn(tokenId);

        // Mint a new BUTT token to the owner of the burned token
        address ownerOfBurned = ownerOf(tokenId);
        uint256 newButtTokenId = mintToken(ownerOfBurned, VALUE_BUTT);

        // Adjust the user's BUTT balance based on the value of the burned token
        // Assuming a function `calculateButtBalanceIncrease` that determines how much
        // the BUTT balance should increase based on the burned token's value
        uint256 buttBalanceIncrease = calculateButtBalanceIncrease(value);
        buttBalance[ownerOfBurned] += buttBalanceIncrease;

        // Emit event or additional logic if needed
    }
    }
*/
